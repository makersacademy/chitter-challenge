feature 'In order to sign up'  do
  ...
  scenario 'I must enter data for name' do
    no_name = User.new(name: '', user_name: 'Johnnyboy', email: 'john@example.com', password: '12345') 
    expect{ sign_up_a(no_name) }.to change(User, :count).by(0)
    expect(page).to have_content "Name must not be blank"
  end
  scenario 'I must enter data for user name' do
    no_username = User.new(name: 'John', user_name: '', email: 'john@example.com', password: '12345') 
  ...
  scenario 'I must enter data for email' do
    no_email = User.new(name: 'John', user_name: 'Johnnyboy', email: '', password: '12345') 
    expect{ sign_up_a(no_email) }.to change(User, :count).by(0)
    expect(page).to have_content "Email must not be blank"
  end
  scenario 'email must be unique' do
    user = User.new(name: 'John', user_name: 'Johnnyboy', email: 'john@example.com', password: '12345') 
    second_user = User.new(name: 'John Doe', user_name: 'Johnny', email: 'john@example.com', password: '12345') 
    sign_up_a user
    expect{ sign_up_a second_user }.to change(User, :count).by(0)
  end
  scenario 'username must be unique' do
    user = User.new(name: 'John', user_name: 'Johnnyboy', email: 'jo@example.com', password: '12345') 
    second_user = User.new(name: 'John Doe', user_name: 'Johnnyboy', email: 'john@example.com', password: '12345') 
    sign_up_a user
    expect{ sign_up_a second_user }.to change(User, :count).by(0)
  end
