feature 'Peeeeeps' do
  ...
  scenario 'without being logged in i can see peeps' do
    visit('/chitter')
    expect(find(:xpath, '//div[@class="peep"][1]//div[@class="content"][1]')).to have_content('haha, does that look funny to you?')
    ...
    expect(find(:xpath, '//div[@class="peep"][5]/div[@class="content"][1]')).to have_content('Bring Sally up: a thread /2')
  ...
  scenario 'after logging in i can see peeps and create a peep' do
    ...
    expect(find(:xpath, '//div[@class="peep"][1]/div[@class="content"][1]')).to have_content('admin')
    ...
    expect(find(:xpath, '//div[@class="peep"][2]/div[@class="content"][1]')).to have_content('haha, does that look funny to you?')
