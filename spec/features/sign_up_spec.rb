feature 'sign up' do
  ...
  scenario 'username not unique' do
    ...
    expect(page).to have_content 'Sorry the username you entered is already taken. Please try another'
  end
  ...
  scenario 'email not unique' do
    ...
    expect(current_path).to eq '/users/new'
    expect(page).to have_content 'Sorry the email you entered is already taken. Please try another'
  end
