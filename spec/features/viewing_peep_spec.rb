feature 'Viewing Peeps' do
  ...
  scenario 'peeps should be visible at /peeps in reverse chronological order' do
    visit '/peeps'
    expect(page).to have_css('main article:first-child', text: 'second peep')
    expect(page).to have_css('main article:last-child', text: 'first peep')
  end

  scenario 'peeps should display the time created on them' do
    visit '/peeps'
    within('main article:last-child') do
