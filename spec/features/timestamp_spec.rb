feature 'Timestamp' do
  ...
  scenario 'User can see what time a peep was posted with peeps in database' do
    ...
    Peep.create(message: "Yesterday's message", timestamp: (Time.now - 86400 ))
    visit '/posts'
    expect(page).to have_content "Message: Today's message #{Time.now.strftime("posted at %I:%M%p on %m/%d/%Y")} Message: Yesterday's message #{(Time.now - 86400).strftime("posted at %I:%M%p on %m/%d/%Y")}"
  end
end
