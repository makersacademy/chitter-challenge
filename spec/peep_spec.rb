...
describe Peep do
  ...
  describe '.all' do
    it 'returns all peeps from database' do
      ...
      described_class.create(text: 'test peep', user_id: user1.id, time: '#{current_timestamp(0)}')
      described_class.create(text: 'another test peep', user_id: user1.id, time: '#{current_timestamp(0)}')
