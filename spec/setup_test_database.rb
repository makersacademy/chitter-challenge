...
def setup_test_database
  ...
  peep_vals = [
    { :peep => 'I wanna rock and roll all night and party every day!', :user => 'Starchild', :name => 'Paul Stanley', :time => '18:00, 13/11/2021' },
    { :peep => "I was made for lovin' you, baby", :user => 'Catman', :name => 'Eric Singer', :time => '18:05, 13/11/2021' },
    { :peep => 'These are crazy, crazy, crazy, crazy nights! Oh yeah!', :user => 'Demon', :name => 'Gene Simmons', :time => '14:00, 14/11/2021' }
  ]
  ...
  peep_vals.each do |peep|
    connection.exec_params("INSERT INTO peeps (peep, peep_user, peep_name, peep_time) values($1,$2,$3,$4);",
    [peep[:peep],peep[:user],peep[:name],peep[:time]]) # Broken over two lines to ease readability
  end

  connection = PG.connect(dbname: 'chitter_zimmja')
  connection.exec('TRUNCATE users; TRUNCATE peeps')
  user_vals.each do |user|
    connection.exec_params("INSERT INTO users (email, pword, fullname, username) values($1,$2,$3,$4);",
    [user[:email],user[:password],user[:fullname],user[:username]]) # Broken over two lines to ease readability
