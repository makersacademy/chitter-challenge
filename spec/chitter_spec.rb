...
describe Chitter do
  ...
  describe "#all" do
    it "Returns a chitter object, which we can extract a message instance from" do
      @test = Chitter.new(:message => "Test 1", :posted_by => "Florence", :created_at => Time.now)
      @test.save
      array = Chitter.all
    ...
    it "Returns a chitter object, which we can extract the time from" do
      @test = Chitter.new(:message => "Test 2", :posted_by => "Florence", :created_at => Time.now)
      @test.save
      array = Chitter.all
      chitter_object = array[@test.id - 1]
    ...
    it "Returns a chitter object, which we can extract the author from" do
      @test = Chitter.new(:message => "Test 3", :posted_by => "Florence", :created_at => Time.now)
      @test.save
      array = Chitter.all
      chitter_object = array[@test.id - 1]
