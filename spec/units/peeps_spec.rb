...
describe Peeps do
  it 'creates a peep with an id and message body' do
      my_peep = Peeps.new(id:1, message: 'This is a test', user_id: 1)
  ...
  it 'can create a peep via .create class method' do
    my_peep = Peeps.create(message: 'This is a test', user_id: 1)
    expect(my_peep).to be_instance_of Peeps
    expect(my_peep.id).to eq '1'
    expect(my_peep.message).to eq 'This is a test'
  end
  ...
  it 'can view peeps via .all method' do
    ...
    expect(Peeps.all.first.id).to eq '1'
    expect(Peeps.all.first.date).to eq ("#{Time.new.strftime('%Y-%m-%d')}")
    expect(Peeps.all.first.time).to eq persisted_data.first['time']
  end
