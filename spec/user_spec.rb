...
describe User do
  describe '.signup' do
    ...
    it 'returns message if email address is already in database' do
      User.signup(name: 'Dave', email: 'dave@dave.com', password: 'pw123', username: 'dave123')
      user2 = User.signup(name: 'Dave', email: 'dave@dave.com', password: 'pw123', username: 'dave123')
      expect(user2).to eq :non_unique_email
    end

    it 'returns message if username is already in database' do
      User.signup(name: 'Dave', email: 'dave@dave.com', password: 'pw123', username: 'dave123')
      user2 = User.signup(name: 'Dave', email: 'dave2@dave.com', password: 'pw123', username: 'dave123')
  ...
  describe '.login' do
    context 'given a user has already signed up' do
      it 'returns a User object containing the username' do
        user = User.signup(name: 'Dave', email: 'dave@dave.com', password: 'pw123', username: 'dave123')
        login_user = User.login(email: 'dave@dave.com', password: 'pw123')
    ...
    context 'given invalid/non-existent login details' do
      it 'nil is returned' do
        expect(User.login(email: 'dave@dave.com', password: 'pw123')).to eq nil
      end
    end
  end
end
