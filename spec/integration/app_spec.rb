...
RSpec.describe Application do
  ...
  describe "get /" do
    context "when not logged in" do
      it "returns non-logged in homepage content" do
        ...
        expect(response.body).to include('<h4>Like Twitter, but legitter!</h4>')
        ...
        expect(response.body).to include('<p> You need to log in if you want to peep! </p>')
    ...
    context "when logged in (also tests post '/')" do
      it "returns logged in homepage content" do
        ...
        expect(response.body).to include('<h3> What would you like to share? </h3>')
        expect(response.body).to include('<form action="/" method="post">')
        expect(response.body).to include('<input type="text" name="content" required>')
  ...
  describe "get /login" do
    it "returns login page" do
      ...
      expect(response.status).to eq (200)
      expect(response.body).to include('<h1>Chitter</h1>')
      expect(response.body).to include('<h4>Log into Chitter</h4>')
      ...
      expect(response.body).to include('</form>')
      expect(response.body).to include('<p><a href="/">Return to homepage</a></p>')
    end
  end
  ...
  describe "get /login_error" do
    it "returns login_error page" do
      ...
      expect(response.status).to eq (200)
      expect(response.body).to include('<h1>Chitter</h1>')
      expect(response.body).to include('<p>Invalid login details</p>')
      expect(response.body).to include('<p><a href="/login">Try again</a></p>')
    end
  end
  ...
  describe "get /signup" do
    it "returns signup page" do
      ...
      expect(response.status).to eq (200)
      expect(response.body).to include('<h1>Chitter</h1>')
      expect(response.body).to include('<h4>Sign up to Chitter</h4>')
  ...
  describe "get /signup_error and post /signup with invalid credentials" do
    context "when entering invalid email address" do
      it "returns invalid email address message" do
        ...
        expect(response.status).to eq (200)
        expect(response.body).to include('<h1>Chitter</h1>')
        expect(response.body).to include('<p><a href="/signup">Try again with a valid email address</a></p>')
      end
    end
    ...
    context "when entering entering email address that's already taken" do
      it "returns message saying email address already taken" do
        ...
        expect(response.status).to eq (200)
        expect(response.body).to include('<h1>Chitter</h1>')
        expect(response.body).to include('<p>That email address has already been taken</p>')
        expect(response.body).to include('<p><a href="/signup">Try again with a different email address</a></p>')
      end
    end
    ...
    context "when entering entering username that's already taken" do
      it "returns message saying username already taken" do
        ...
        expect(response.status).to eq (200)
        expect(response.body).to include('<h1>Chitter</h1>')
        expect(response.body).to include('<p>That username has already been taken</p>')
        expect(response.body).to include('<p><a href="/signup">Try again with a different username</a></p>')
      end
    end
    ...
    context "when entering entering email address and username that are both already taken" do
      it "returns message saying email address and username both already taken" do
        ...
        expect(response.status).to eq (200)
        expect(response.body).to include('<h1>Chitter</h1>')
        expect(response.body).to include('<p>The email address and username you provided have both already been taken</p>')
        expect(response.body).to include('<p><a href="/signup">Try again using other details</a></p>')
      end
    end
  end
  ...
  describe "get '/:user_id' (view a user's profile)" do
    context "when not logged in" do
      it "returns profile and peeps of user with given user_id" do
        ...
        expect(response.body).to include('<a href="/login">Log in</a> |')
        expect(response.body).to include('<a href="/signup">Sign up</a>')
        expect(response.body).to include('<p> You need to log in if you want to peep or comment! </p>')
        expect(response.body).to include("Luigi_100")
        ...
        expect(response.body).to include('<p><a href="/peep/3"> view comments </a></p>')
      end
    end
    ...
    context "when logged in and visiting someone else's page" do
      it "returns profile and peeps of user with given user_id" do
        ...
        expect(response.body).to include('Logged in as')
        expect(response.body).to include('Luigi_100')
        expect(response.body).to include('<a href="/logout">Log out and return to homepage</a>')
        ...
        expect(response.body).to include('<p><a href="/peep/5"> view comments </a></p>')
      end
    end
    ...
    context "when logged in and visiting own page" do
      it "returns user's own profile and peeps" do
        ...
        expect(response.body).to include('Luigi_100')
        expect(response.body).to include('<a href="/logout">Log out and return to homepage</a>')
        expect(response.body).to include('<h2> What would you like to share? </h2>')
        expect(response.body).to include('<form action="/" method="post">')
        expect(response.body).to include('<input type="text" name="content" required>')
        expect(response.body).to include('<input type="submit" value="Post peep">')
        expect(response.body).to include('</form>')
  ...
  describe "get '/peep/:peep_id' (view comments)" do
    ...
    context "when logged in" do
      it "returns the page with a post comment function" do
        ...
        expect(response.body).to include('2022-12-01 19:34:54')
        expect(response.body).to include('Did you leave it in the bath again??')
        expect(response.body).to include('<h3> Would you like to comment on this peep? </h3>')
        expect(response.body).to include('<input type="text" name="content" required>')
