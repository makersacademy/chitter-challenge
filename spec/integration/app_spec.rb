...
describe Application do
  ...
  context 'GET /sign-up' do
    it 'shows a form for user to sign up' do
      ...
      expect(response.status).to eq 200
      expect(response.body).to include "<form class='form' method='POST' action='/new-user'>"
    end
  end
  ...
  context 'POST /new-user' do
    ...
    it 'shows error message for invalid username' do
      ...
      expect(response.status).to eq 200
      expect(response.body).to include '<p class="error_message">No special characters or space allowed in username.</p>'
    end
    ...
    it 'shows error message for invalid password length' do
      ...
      expect(response.status).to eq 200
      expect(response.body).to include '<p class="error_message">Password must be at least 6 characters.</p>'
    end
    ...
    it 'shows error message for exist username' do
      ...
      expect(response.status).to eq 200
      expect(response.body).to include '<p class="error_message">Username exists!</p>'
    end
    ...
    it 'shows error message for exist email' do
      ...
      expect(response.status).to eq 200
      expect(response.body).to include '<p class="error_message">Email exists!</p>'
    end
  end
  ...
  context 'POST /user-login' do
    ...
    it 'displays a form for logged-in users' do
      ...
      expect(last_response.body).to include '<textarea class="textarea" name="content" rows="4" cols="50" placeholder="Luke, What\'s happening?"></textarea>'        
      expect(last_response.body).to include '<input class="btm " type="submit" value="Post">'
    end
  ...
  context 'POST /logout' do
    it 'logouts user session' do
      ...
      expect(response.status).to eq 302
      follow_redirect!
      expect(last_response.body).not_to include '<textarea class="textarea" name="content" rows="4" cols="50" placeholder="Luke, What\'s happening?"></textarea>'
      expect(last_response.body).to include '<a href="/sign-up" class="btm">SIGN UP</a>'
