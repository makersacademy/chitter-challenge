...
describe User do
  ...
  it 'Can force email true' do
    ...
    expect(add_user(user)).to include("Email is required.")
  end

  it 'Can force email address to be correct format' do
    user = build(:user, email: "waef")
    expect(add_user(user)).to include("Please enter a valid email address")
  end

  it 'Can force email unique' do
    user = build(:user)
    add_user(user)
    expect(add_user(user)).to include("Email address is taken")
  end

  it 'Can force name true' do
    user = build(:user, real_name: "")
    expect(add_user(user)).to include("Name is required.")
  end

  it 'Can force username true' do
    user = build(:user, user_name: "")
    expect(add_user(user)).to include("Username is required.")
  end

  it 'can force username unique' do
    user = build(:user)
    add_user(user)
    expect(add_user(user)).to include("Username is taken")
  end

  it 'Can not add user with incorrect passwords' do
    user = build(:user, password: "ewfa")
    expect(add_user(user)).to include("Passwords do not match.")
  end

  it 'Does not allow password to be blank' do
    user = build(:user, password: "")
    expect(add_user(user)).to include("Password cannot be blank")
  end
