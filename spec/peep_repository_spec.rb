...
describe PeepRepository do
  ...
  context "when returning all Peep objects" do
    it "returns only Peep objects" do
      ...
      expect(peeps[4].time).to eq '2008-10-19 10:15:54'
  ...
  context "when looking for a peep by ID" do
    it "returns one Peep object" do
      ...
      expect(peep.time).to eq '2005-10-19 12:23:54'
  ...
  it "create a Peep object" do
    time_double = Time.new(2004, 10, 19, 7, 23, 7)
    repo = PeepRepository.new
    new_peep = Peep.new
    new_peep.content = 'a very new content'
    new_peep.time = time_double
    new_peep.user_id = 2
    ...
    expect(added_peep.time).to eq '2004-10-19 07:23:07'
