class Tags
  ...
  def self.create(id:, tag:)
    ...
    tagcheck = DatabaseConnection.query("SELECT * FROM tags
      WHERE user_id = '#{user.id}' AND peep_id = '#{id}';")
    return "Already Tagged" if tagcheck.any?

    result = DatabaseConnection.query("INSERT INTO tags (user_id, peep_id)
    VALUES(#{user.id}, #{id}) RETURNING id, peep_id, user_id;").first
    Tags.new(id: result['id'], user_id: result['user_id'],
      peep_id: result['peep_id'])
  end
  ...
  def self.find(peep_id:)
    ...
    Tags.new(id: result[0]['id'], user_id: result[0]['user_id'],
  ...
  def self.find_by_user_id(user_id:)
    ...
    result.map do |peep|
      Tags.new(id: peep['id'], user_id: peep['user_id'],
