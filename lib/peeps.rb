...
class Peeps
  ...
  def self.all
    peeps = sql('
                      ...
                      on u.id = user_id
                      order by time desc;')
    peeps.map { |peep| Peeps.new(peep['id'],
                                peep['user_id'],
                                peep['message'],
  ...
  def self.create(user_id, peep_content, replying_to="NULL")
    new_peeps = sql("insert into peeps(user_id, message, replying_to_id)
          values (#{user_id}, '#{peep_content}', #{replying_to})
          RETURNING id, user_id, message, time, replying_to_id;")
          new_peep = new_peeps[0]
          new_peep = Peeps.new(new_peep["id"],
                    new_peep["user_id"],
                    new_peep["message"],
                    new_peep["time"],
                    new_peep["replying_to_id"]
                    )
  ...
  def update_tags
    ...
    tags.each do |tag|
      ...
      p "tag stripped is #{tag_stripped}"
