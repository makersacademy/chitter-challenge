...
user_id: peep['user_id'])
  ...
  def self.create(peep:, user_id:)
    ...
    query = "INSERT INTO peeps (peep, user_id) VALUES ('#{escaped_peep}', #{user_id}) RETURNING peep_id, peep, created_at, user_id"
    result = DatabaseConnection.query(query) 
    result.map do 
      |peep| Peep.new(id: peep['peep_id'], peep: peep['peep'], created_at: peep['created_at'], 
user_id: peep['user_id'])
    end.first
  end
