...
class Peep
  ...
  def self.create(peep, maker = nil)
    rs = DatabaseConnection.connection.exec_params("INSERT INTO peeps (peep, maker) VALUES ($1, $2) 
      RETURNING id, peep, maker;", [peep, maker])
    Peep.new(id: rs[0]['id'], peep: rs[0]['peep'], 
      maker: rs[0]['maker'], created_at: rs[0]['created_at'])
  end
