...
class Peep
  ...
  def self.all
    ...
    peeps = result.map do |peep| 
      Peep.new(id: peep['id'], content: peep['content'], user: peep['username'], handle: peep['userhandle'], time: peep['time']) 
    end 
  end
  ...
  def self.add(content: , user: , handle: )
    ...
    result = connection.exec_params("INSERT INTO peeps (content, username, userhandle, time) VALUES($1, $2, $3, $4) RETURNING id, content, username, userhandle, time;", [content, user, handle, time])
