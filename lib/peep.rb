...
class Peep
  ...
  def self.create(message:, user_id:)
    result = DatabaseConnection.query("INSERT INTO peeps (message, user_id) VALUES(#{DatabaseConnection.connection.escape_literal(message)},
    #{user_id}) RETURNING id, message, time, user_id;")
