...
class Peep
  ...
  def self.create(body:, user_id:)
    ...
    results = DatabaseConnection.query(
      "INSERT INTO peeps (body, user_id)
       VALUES ('#{CGI.escapeHTML(body)}', '#{user_id}')
       RETURNING id, body, datetimeposted, user_id;")
    
    author = DatabaseConnection.query(
      "SELECT username FROM users
      WHERE id='#{results[0]['user_id']}';")[0]['username']

    Peep.new(
      id: results[0]['id'],
      time: DateTime.parse(results[0]['datetimeposted']),
      body: results[0]['body'],
      author: author)
  end
  ...
  def self.all
    ...
    results.map do |peep|
      author = DatabaseConnection.query(
        "SELECT username FROM users
        WHERE id='#{peep['user_id']}';"
        )[0]['username']
        
      Peep.new(
        id: peep['id'],
        time: DateTime.parse(peep['datetimeposted']),
        body: peep['body'],
        author: author)
    end
  end
  ...
  def favourited?(user_id)
    !DatabaseConnection.query(
      ...
      AND peep_id = #{id};").num_tuples.zero?
  end
end
