...
class User
  ...
  def self.add(email:, password:, username:, name:)
    safe_password = BCrypt::Password.create(password)
    data = DatabaseConnection.query("INSERT INTO users (email, password, username, name) 
      VALUES('#{email}', '#{safe_password}', '#{username}', '#{name}') 
