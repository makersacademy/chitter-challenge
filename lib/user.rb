class User
  ...
  def self.all
    ...
    users.map do |user_hash|
      User.new(id: user_hash['id'], email: user_hash['email'], username: user_hash['username'], password: user_hash['password'])
    end
  end
  ...
  def self.create(email:, username:, password:)
    ...
    User.new(id: user['id'], email: user['email'], username: user['username'], password: user['password'])
  end
