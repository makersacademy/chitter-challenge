...
class ChitterApp
  ...
  def self.all
    connection = PG.connect(dbname: 'chitter_machine_test', user: 'postgres', password: 'password')
    result = connection.exec('SELECT chitter_message, chitter.created_at, username FROM chitter INNER JOIN students ON (chitter.user_id = students.user_id) ORDER BY chitter.created_at DESC;')

    # No idea why this doesn't work, works fine in the terminal....
