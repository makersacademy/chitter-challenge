...
class Cheet
  ...
  def self.all
    ...
    unsorted = result.map do |cheet|
      Cheet.new(id: cheet['id'],
        content: cheet['content'],
        timestamp: cheet['timestamp'])
    end
    sorted = unsorted.sort_by { |cheet| cheet.timestamp }
    sorted.reverse
  end
