class Replies
  ...
  def self.create(peep_id:, message:, user_id:)
    result = DatabaseConnection.query("INSERT INTO
      replies (peep_id, message, user_id, date_time)
      VALUES(#{peep_id}, '#{message}', #{user_id},
        '#{Time.now.strftime("%d/%m/%Y %H:%M")}')
        RETURNING id, peep_id, message, user_id, date_time;")
    Replies.new(id: result[0]['id'], peep_id: result[0]['peep_id'],
      message: result[0]['message'], user_id: result[0]['user_id'],
  ...
  def self.find(peep_id:)
    ...
    result.map do |reply|
      Replies.new(id: reply['id'], peep_id: reply['peep_id'],
        message: reply['message'], user_id: reply['user_id'],
