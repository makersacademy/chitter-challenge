...
| Refactor DBConnection so that connection and disconnection are done automatically by the run_query method | low | As with the validation refactor, this would make the code easier to maintain, but for an app of this size this is less important |
...
| Change get /sessions/destroy to post /sessions/destroy | low | A consistency/semantics issue with the way the route was designed. Little impact on the site.