...
class Peep
  ...
  def self.all
    ...
    result = DatabaseConnection.query('SELECT * FROM peeps_data order by time desc;')
  ...
  def self.create(peep:, time: DateTime.now)
    ...
    result = DatabaseConnection.query("INSERT INTO peeps_data (peep, time) VALUES('#{peep}', '#{time}') RETURNING id, peep, time;")
