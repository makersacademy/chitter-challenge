...
users: username, password, email_address 
```
...
| Record                | Properties          |
| --------------------- | ------------------  |
| peeps                 | peep_content, time_posted, user_id
| users                 | username, password, user_email

Name of the first table (always plural): peeps

Column names: peep_content, time_posted, user_id

Name of the second table (always plural): users

Column names: username, password, user_email
...
```bash
psql -h 127.0.0.1 database_name < users_peeps_table.sql
```
